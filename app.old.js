            ...
            if ( typeof infos != 'undefined') {
                ...
                if (req.body.date_arrive<=req.body.date_depart) {
                    ...
                    mysqlConnection.query(sql, (err, rows, fields) => {
                        ...
                        mysqlConnection.query(sql, (err, rows, fields) => {
                            ...
                            mysqlConnection.query(sql, (err, rows, fields) => {
                                ...
                                mysqlConnection.query(sql, (err, rows, fields) => {
                                    var id_infos = rows.insertId;
                                    console.log(id_infos);
                                ...
                                if (IDCHAMBRE.length === PRIX.length){
                                    let idchambre=0;
                                    let prix=0 ;
                                    for (let j =0 ;j < IDCHAMBRE.length; j++){
                                        idchambre = IDCHAMBRE[j];
                                        prix = PRIX[j];
                                        console.log(idchambre, prix,'test');
                                        var sql = "select * from chambre where id_chambre = "  + IDCHAMBRE[j]+ " ";
                                        mysqlConnection.query(sql, (err, rows, fields) => {
                                            // var i=0;
                                            if (rows[0].categorie === 'chambre standart') {
                                                ...
                                                // }
                                                if (prix >= 15000) {
                                                    var sql = "insert into chambreclient(id_client, id_chambre, status_ch,montant,date) values( "  + insertid + "," + IDCHAMBRE[j] + ",'" + 1 + "'," + PRIX[j]+ ",'" + toDay +  "')";
                                                    console.log(sql);
                                                    mysqlConnection.query(sql, (err, rows, fields) => {
                                                        var sql = "insert into log(id_user, action, id_client,id_commande,id_sortie,id_entree,id_infos,id_chambre,created_at) values(" + req.session.userid + ",'enregistrement  chambre',"+null+","+null+","+null+","+null+","+null+","+IDCHAMBRE[j]+",'" + toDay + "')";
                                                        mysqlConnection.query(sql, (err, rows, fields) => {})
                                                    })
                                                    var sql = "UPDATE  chambre SET status = 'occupé' WHERE id_chambre = " +  IDCHAMBRE[j] ;
                                                    console.log(sql);
                                                    mysqlConnection.query(sql, (err, rows, fields) => {
                                                    ...
                                                    var sql = "UPDATE `chambre` SET `status` = 'occupé' WHERE `chambre`.`id_chambre` = " + IDCHAMBRE[j] + "";
                                                    console.log(sql);
                                                    mysqlConnection.query(sql, (err, rows, fields) => {
                                            ...
                                            if (rows[0].categorie === 'chambre confort' ) {
                                                ...
                                                if (prix >= 20000) {
                                                    ...
                                                    var sql = "insert into chambreclient(id_client, id_chambre, status_ch,montant,date) values( "  + insertid + "," + IDCHAMBRE[j] + ",'" + 0 + "'," + PRIX[j] + ",'" + toDay +  "')";
                                ...
                                //   }
                                //
                                //   if ( req.body.chambre[0]>=1 && req.body.chambre[0]<=3) {
                                //
                                //
                                //
                                //
                    ...
                    if (req.session.role === 'receptionniste') {
                        console.log('icci receptioniste');
                        var message = 'client enregitré avec succès';
                        var sql = "select * from client order by id_client asc";
                        mysqlConnection.query(sql, (err, rows, fields) => {
                            let client = rows;
